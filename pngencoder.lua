...
function Png:crc32(data, index, len)
    ...
    for i=index,index+len-1 do
        local byte = data[i]
        for j=0,7 do  -- Inefficient bit32wise implementation, instead of table-based
            local nbit32 = bit32.band(bit32.bxor(self.crc, bit32.rshift(byte, j)), 1);
            self.crc = bit32.bxor(bit32.rshift(self.crc, 1), bit32.band((-nbit32), 0xEDB88320));
        end
    end
    self.crc = bit32.bnot(self.crc)
end
